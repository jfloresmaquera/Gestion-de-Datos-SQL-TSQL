...
IF OBJECT_ID('StarTeam.BI_Obtener_Tiempo_ID') IS NOT NULL
	DROP FUNCTION StarTeam.BI_Obtener_Tiempo_ID
GO
...
IF OBJECT_ID('StarTeam.BI_Migrar_HECHO_Camion_Cuatrimestre') IS NOT NULL
  DROP PROCEDURE StarTeam.BI_Migrar_HECHO_Camion_Cuatrimestre
...
IF OBJECT_ID('StarTeam.VIEW_Costo_Mantenimiento_X_Camion_X_Taller_X_Cuatrimestre') IS NOT NULL
  DROP VIEW StarTeam.VIEW_Costo_Mantenimiento_X_Camion_X_Taller_X_Cuatrimestre
  
IF OBJECT_ID('StarTeam.VIEW_Costo_Mantenimiento_X_Camion') IS NOT NULL
  DROP VIEW StarTeam.VIEW_Costo_Mantenimiento_X_Camion

IF OBJECT_ID('StarTeam.VIEW_Desvio_Promedio') IS NOT NULL
  DROP VIEW StarTeam.VIEW_Desvio_Promedio

IF OBJECT_ID('StarTeam.VIEW_Tareas_Por_Modelo_Camion') IS NOT NULL
  DROP VIEW StarTeam.VIEW_Tareas_Por_Modelo_Camion

IF OBJECT_ID('StarTeam.VIEW_Materiales_Por_Taller') IS NOT NULL
  DROP VIEW  StarTeam.VIEW_Materiales_Por_Taller
...
CREATE TABLE StarTeam.BI_Orden_Trabajo (
  ...
  BI_ORDEN_TRABAJO_CUATRIMESTRE int FOREIGN KEY REFERENCES StarTeam.BI_Tiempo(BI_TIEMPO_ID) NOT NULL
);
GO
...
CREATE FUNCTION StarTeam.BI_Obtener_Tiempo_ID(@ORDEN_TRABAJO_FECHA nvarchar(255))
RETURNS int
BEGIN 
	DECLARE @tiempo_ID as int

	SELECT @tiempo_ID = BI_TIEMPO_ID from StarTeam.BI_Tiempo
	where year(CAST(@ORDEN_TRABAJO_FECHA AS DATETIME2)) = BI_TIEMPO_ANIO
	and StarTeam.BI_Obtener_Cuatrimestre(CAST(@ORDEN_TRABAJO_FECHA AS DATETIME2)) = BI_TIEMPO_CUATRIMESTRE
...
AS
BEGIN
 INSERT INTO StarTeam.BI_Orden_Trabajo (BI_ORDEN_TRABAJO_NUMERO, 
                                        BI_ORDEN_TRABAJO_CAMION, 
                                        BI_ORDEN_TRABAJO_TALLER_NRO, 
                                        BI_ORDEN_TRABAJO_ESTADO, 
                                        BI_ORDEN_TRABAJO_FECHA, 
                                        BI_ORDEN_TRABAJO_CUATRIMESTRE)
  SELECT DISTINCT ORDEN_TRABAJO_NUMERO, 
                  ORDEN_TRABAJO_CAMION, 
                  ORDEN_TRABAJO_TALLER_NRO, 
                  ORDEN_TRABAJO_ESTADO, 
                  ORDEN_TRABAJO_FECHA, 
                  StarTeam.BI_Obtener_Cuatrimestre(ORDEN_TRABAJO_FECHA)
  FROM StarTeam.Orden_Trabajo
END
GO
...
CREATE PROCEDURE StarTeam.BI_Migrar_HECHO_Camion_Cuatrimestre
AS
BEGIN
  INSERT INTO StarTeam.BI_HECHO_Camion_Cuatrimestre(BI_CAMION_ID,
													BI_CUATRIMESTRE_ID,
													BI_MAX_TIEMPO_FUERA_SERVICIO)
  SELECT DISTINCT C.BI_CAMION_NUMERO,
				  StarTeam.BI_Obtener_Tiempo_ID(OT.BI_ORDEN_TRABAJO_FECHA),
				  (SELECT TOP 1 SUM(txot.BI_TAREA_TIEMPO_EJECUTADO)
					FROM StarTeam.BI_Tarea_X_Orden_Trabajo txot
						JOIN StarTeam.BI_Orden_Trabajo ot ON ot.BI_ORDEN_TRABAJO_NUMERO = txot.BI_TAREA_ORDEN_TRABAJO
					WHERE ot.BI_ORDEN_TRABAJO_CAMION = C.BI_CAMION_NUMERO
						AND StarTeam.BI_Obtener_Tiempo_ID(ot.BI_ORDEN_TRABAJO_FECHA) = StarTeam.BI_Obtener_Tiempo_ID(OT.BI_ORDEN_TRABAJO_FECHA)
					GROUP BY ot.BI_ORDEN_TRABAJO_CAMION, StarTeam.BI_Obtener_Tiempo_ID(ot.BI_ORDEN_TRABAJO_FECHA)
					ORDER BY SUM(BI_TAREA_TIEMPO_EJECUTADO))
  FROM StarTeam.BI_Camion C
  JOIN StarTeam.BI_Orden_Trabajo OT ON OT.BI_ORDEN_TRABAJO_CAMION = C.BI_CAMION_NUMERO
  GROUP BY C.BI_CAMION_NUMERO,StarTeam.BI_Obtener_Tiempo_ID(OT.BI_ORDEN_TRABAJO_FECHA)
  ORDER BY C.BI_CAMION_NUMERO,StarTeam.BI_Obtener_Tiempo_ID(OT.BI_ORDEN_TRABAJO_FECHA) ASC
END
GO
...
EXEC StarTeam.BI_Migrar_HECHO_Costo_Orden_Trabajo
...
/*-----------------------------------------------VISTAS-------------------------------------------------*/
 ...
 /*Costo total de mantenimiento por camión, por taller, por cuatrimestre.
  ...
  CREATE VIEW StarTeam.VIEW_Costo_Mantenimiento_X_Camion_X_Taller_X_Cuatrimestre AS

  SELECT DISTINCT ot.BI_ORDEN_TRABAJO_CAMION, 
                  ot.BI_ORDEN_TRABAJO_TALLER_NRO, 
                  BI_ORDEN_TRABAJO_CUATRIMESTRE, 
                  SUM(hot.BI_COSTO_ORDEN_TRABAJO) BI_COSTO_TOTAL
	FROM StarTeam.BI_Orden_Trabajo ot 
  JOIN StarTeam.BI_HECHO_Costo_Orden_Trabajo hot ON hot.BI_ORDEN_TRABAJO_NUMERO = ot.BI_ORDEN_TRABAJO_NUMERO
	GROUP BY ot.BI_ORDEN_TRABAJO_CAMION, ot.BI_ORDEN_TRABAJO_TALLER_NRO, BI_ORDEN_TRABAJO_CUATRIMESTRE
 
  GO

  /*Costo de Mantenimiento*/

  CREATE VIEW StarTeam.VIEW_Costo_Mantenimiento_X_Camion AS
  
  SELECT BI_ORDEN_TRABAJO_CAMION, SUM(BI_COSTO_TOTAL) BI_MANTENIMIENTO_X_CAMION
  FROM StarTeam.VIEW_Costo_Mantenimiento_X_Camion_X_Taller_X_Cuatrimestre
  GROUP BY BI_ORDEN_TRABAJO_CAMION

  GO


  --Desvío promedio de cada tarea x taller (completa)
CREATE VIEW StarTeam.VIEW_Desvio_Promedio AS
select BI_TALLER_NOMBRE, BI_TAREA_DESCRIPCION, avg(BI_TAREA_TIEMPO_EJECUTADO - BI_TAREA_TIEMPO_ESTIMADO) as 'Desvio' from StarTeam.BI_Taller taller
join StarTeam.BI_Orden_Trabajo ot on taller.BI_TALLER_NUMERO = ot.BI_ORDEN_TRABAJO_TALLER_NRO
join StarTeam.BI_Tarea_X_Orden_Trabajo txot on ot.BI_ORDEN_TRABAJO_NUMERO = txot.BI_TAREA_ORDEN_TRABAJO
join StarTeam.BI_Tarea tarea on tarea.BI_TAREA_CODIGO = txot.BI_TAREA_CODIGO
group by BI_TAREA_DESCRIPCION, BI_TALLER_NOMBRE
--order by BI_TALLER_NOMBRE, BI_TAREA_DESCRIPCION
GO


--Las 5 tareas que más se realizan por modelo de camión (incompleta)
CREATE VIEW StarTeam.VIEW_Tareas_Por_Modelo_Camion AS
select BI_MODELO_CAMION_DESCRIPCION, BI_TAREA_DESCRIPCION, count(t.BI_TAREA_CODIGO) as 'Cantidad de tareas' from Starteam.BI_Tarea t
join StarTeam.BI_Tarea_X_Orden_Trabajo txot on t.BI_TAREA_CODIGO = txot.BI_TAREA_CODIGO
join StarTeam.BI_Orden_Trabajo ot on txot.BI_TAREA_ORDEN_TRABAJO = ot.BI_ORDEN_TRABAJO_NUMERO
join StarTeam.BI_Camion c on ot.BI_ORDEN_TRABAJO_CAMION = c.BI_CAMION_NUMERO
join StarTeam.BI_Modelo m on c.BI_CAMION_MODELO = m.BI_MODELO_NUMERO
join StarTeam.BI_Modelo_Camion mc on m.BI_MODELO_NUMERO = mc.BI_MODELO_CAMION_ID
group by BI_MODELO_CAMION_DESCRIPCION, BI_TAREA_DESCRIPCION
--order by BI_MODELO_CAMION_DESCRIPCION, [Cantidad de tareas] DESC;
GO


--Los 10 materiales más utilizados por taller (incompleta)
CREATE VIEW StarTeam.VIEW_Materiales_Por_Taller AS
select BI_TALLER_NOMBRE, BI_MATERIAL_DESCRIPCION, count(m.BI_MATERIAL_CODIGO) as 'Cantidad de materiales' from StarTeam.BI_Material m
join StarTeam.BI_Material_X_Tarea mxt on m.BI_MATERIAL_CODIGO = mxt.BI_MATERIAL_CODIGO
join StarTeam.BI_Tarea tarea on mxt.BI_TAREA_CODIGO = tarea.BI_TAREA_CODIGO 
join StarTeam.BI_Tarea_X_Orden_Trabajo txot on tarea.BI_TAREA_CODIGO = txot.BI_TAREA_CODIGO
join StarTeam.BI_Orden_Trabajo ot on txot.BI_TAREA_ORDEN_TRABAJO = ot.BI_ORDEN_TRABAJO_NUMERO
join StarTeam.BI_Taller taller on ot.BI_ORDEN_TRABAJO_TALLER_NRO = taller.BI_TALLER_NUMERO
group by BI_TALLER_NOMBRE, BI_MATERIAL_DESCRIPCION
--order by BI_TALLER_NOMBRE, [Cantidad de materiales] DESC
GO
